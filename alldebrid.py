...
# def check_instant_availability(magnet_uri, retries=5, backoff_factor=0.5, max_backoff=16, jitter_factor=0.5):
...
#             backoff = min(backoff_factor * (2 ** i), max_backoff)
...
#     else:
#         # Retry limit reached, raise error
#         raise ValueError(f"Request failed after {retries} retries")
    
# def check_cached_instant_availabilities(magnet_uris):
#     return list(executor.map(check_instant_availability, magnet_uris))
    
# async def check_instant_availability_async(session, magnet_uri, retries=5, backoff_factor=0.5, max_backoff=16, jitter_factor=0.5):
#     assert magnet_uri, "Magnet URI (or hash) is required. Please provide a magnet URI or hash."
...
#         try:
#             async with session.get(INSTANT_AVAILABILITY_URL, headers=headers, params=params) as response:
#                 if response.status == 200:
#                     response_json = await response.json()
#                     return response_json
#                 elif response.status == 503:
#                     if i == retries:
#                         raise ValueError(f"Request failed with error code {response.status} after {retries} retries")
#                     else:
#                         backoff = min(backoff_factor * (2 ** i), max_backoff)
#                         jitter = random.uniform(0, jitter_factor * backoff)
#                         sleep_time = backoff + jitter
...
#                 else:
#                     raise ValueError(f"Request failed with error code {response.status}")
#         except aiohttp.ClientError as exc:
#             if i == retries:
#                 raise ValueError(f"Request failed due to client error: {exc}") from exc
#             else:
#                 backoff = min(backoff_factor * (2 ** i), max_backoff)
#                 jitter = random.uniform(0, jitter_factor * backoff)
#                 sleep_time = backoff + jitter
#                 await asyncio.sleep(sleep_time)
# async def check_cache_availability_async(items: list) -> list:
#     async with aiohttp.ClientSession() as session:
#         tasks = []
#         for item in items:
#             magnet_uri = item.get("links", [])[0]
#             task = asyncio.ensure_future(check_instant_availability_async(magnet_uri=magnet_uri, session=session))
#             tasks.append(task)

#         cache_response = await asyncio.gather(*tasks)

#         for item, cache_response in zip(items, cache_response):
#             item["cached"] = cache_response.get("data", {}).get("magnets", [])[0].get("instant", False)
#     return items
...
# # import json
# # with open('results/result_hd4k_S01_orionoid.json', 'r', encoding='utf-8') as file:
# #     results = json.load(file)

# # updated_results = asyncio.run(check_cache_availability_async(results))
